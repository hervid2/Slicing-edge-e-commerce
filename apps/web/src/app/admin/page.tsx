...
export default async function AdminDashboardPage() {
  ...
  try {
    metrics = await getAdminMetrics(token);
  } catch {
    metricsError = true;
  }
  ...
  const kpis = [
    {
      label: 'Total Orders',
      value: metrics?.kpis.totalOrders ?? '—',
      sub: metrics ? `${metrics.kpis.pendingOrders} pending` : '',
      href: '/admin/orders',
    },
    {
      label: 'Revenue',
      value: metrics ? formatCurrency(metrics.kpis.totalRevenue) : '—',
      sub: 'Excl. cancelled',
      href: '/admin/orders',
    },
    {
      label: 'Active Products',
      value: metrics?.kpis.activeProducts ?? '—',
      sub: '',
      href: '/admin/products',
    },
    {
      label: 'Users',
      value: metrics?.kpis.totalUsers ?? '—',
      sub: '',
      href: '/admin/users',
    },
  ];
