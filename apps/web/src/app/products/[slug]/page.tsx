        ...
        <section className="mt-16">
          ...
          <div className="mt-6 space-y-6">
            {product.reviews.map((review) => (
              <div key={review.id} className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--color-background)] font-semibold text-[var(--color-primary)]">
                      {review.user.name?.charAt(0) || '?'}
                    </div>
                    <div>
                      <p className="font-medium text-[var(--color-foreground)]">{review.user.name || 'Anonymous'}</p>
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <svg
                            key={star}
